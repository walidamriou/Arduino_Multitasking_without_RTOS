...
/* ===== pfod Command for Menu_1 ====
  pfodApp msg {.} --> {,<bg n><b><+3><w>~SMS Starter`0~V19|A<bg bl>`0~D4 output is ~~Low\High~|B<bg bl>`0~D5 PWM Setting ~%`255`0~100~0~|!F<-6>~|!C<bg bl>`1~D6 Input is ~~Low\High~t|!E<-6>~|!D<bg bl>`775~A0 ~V`1023`0~5~0~}
*/
// Using Mega2560 Serial1 and 19200 baud for send and receive to
// Adafruit FONA 3G SIM5320 Cellular Breakout
// This code allows Serial (USB) to be used for DEBUG
// NOTE: Because of the RAM requirements, this code needs to be loaded onto a Arduino Mega 2560 or similar
// NOTE: default baud rate for SIM5320 is 115200. This needs to changed to 19200 before use.

/* Code generated by pfodDesignerV3 V3.0.3303
 */
/*
...
// see https://www.forward.com.au/pfod/secureChallengeResponse/keyGenerator/index.html for a QR image key generator
// and https://www.forward.com.au/pfod/secureChallengeResponse/index.html for more information
int swap01(int); // method prototype for slider end swaps

pfodSMS_SIM5320 pfodSMS; // create object to handle SMS messages
pfodSecurity parser("V5"); // create a parser to handle the pfod messages
...
//  to set debugOut in pfodSMS_SIM5320
//  Serial.begin(19200);
//  pfodSMS.setDebugStream(&Serial); // need to uncomment a #define DEBUG... in the pfodSMS_SIM5320.cpp file as well to get debug output

  // initialize the SMS and connect the parser
  pfodSMS.init(&Serial1); //  just 4 pins used power and TX/RX
  parser.connect(&pfodSMS,F(pfodSecurityCode)); // connect parser to SMS stream
...
void sendMainMenu() {
  ...
  parser.print(F("<bg n><b><+3><w>~SMS Starter`0"));
