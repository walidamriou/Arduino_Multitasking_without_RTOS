...
int reading = 100; // value for guage in Tonne
...
unsigned long guageTimer;
const unsigned long GUAGE_TIMER_INTERVAL = 1000; // 1000mS
unsigned int guageIncrement = 5;
...
void setup() {
  ...
  guageSetup(); // set up guage
  guageSetValue(reading);  // set intial reading
  guageTimer = millis(); // initialze timer
}
...
void loop() {
  ...
  if (nowMs - guageTimer >= GUAGE_TIMER_INTERVAL) {
    guageTimer = nowMs;
    reading = reading + guageIncrement;
    if (reading >= 1000) {
      reading == 1000;
      guageIncrement = -guageIncrement;
    }
    if (reading <= 0) {
      reading = 0;
      guageIncrement = -guageIncrement;
    }
    guageSetValue(reading);
  }
}
...
void sendDrawingUpdates_z() {
  ...
  guageUpdate(); // update with current state
  dwgs.end();
}
...
// ====================== this is the guage dwg code =================
// modified from the guage example in pfodDwgControls

int  guage_z_idx = pfodDwgs::reserveIdx(3);
const __FlashStringHelper* guageLabel;
int guageValue = 0; // 0 to 1000
...
void guageSetValue(int value) {
  guageValue = value;
}

void guageDraw() {
  dwgs.arc().filled().color(dwgs.GREY).radius(5).start(217.5).angle(-255).send();
  dwgs.arc().filled().color(dwgs.WHITE).radius(4).idx(guage_z_idx + 1).start(217.5).angle(-255).send();
...
void guageUpdate() {
  float guageAngle = guageValue / 1000.0 * 255.0; // scaled to 0 to 255 for display angle
  dwgs.arc().filled().color(dwgs.RED).radius(5).idx(guage_z_idx).start(217.5).angle(-guageAngle).send();
  dwgs.label().color(dwgs.RED).idx(guage_z_idx + 2).fontSize(-3).bold().text(guageLabel).intValue(guageValue).maxValue(1000).displayMax(1000).decimals(0).units(F("t")).send();
}
